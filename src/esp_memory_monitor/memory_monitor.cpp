...
MemoryScope::MemoryScope(
    ...
    std::string name,
    MemoryTag tag,
    size_t startInternal,
    size_t startPsram,
    uint64_t startUs
)
    : _monitor(monitor), _name(std::move(name)), _tag(tag), _startInternal(startInternal),
      _startPsram(startPsram), _startUs(startUs) {
}
...
MemorySnapshot ESPMemoryMonitor::sampleNow() {
	...
	MemoryMonitorVector<ThresholdEvent> events{
	    MemoryMonitorAllocator<ThresholdEvent>(_usePSRAMBuffers)
	};
	MemoryMonitorVector<TaskStackEvent> stackEvents{
	    MemoryMonitorAllocator<TaskStackEvent>(_usePSRAMBuffers)
	};
...
MemoryScope ESPMemoryMonitor::beginScope(const std::string &name, MemoryTag tag) {
	...
	const RegionStats internal =
	    captureRegion(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT, MemoryRegion::Internal);
	const RegionStats psram =
	    captureRegion(MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT, MemoryRegion::Psram);
...
bool ESPMemoryMonitor::setTaskStackThreshold(
    const std::string &taskName, const TaskStackThreshold &threshold
) {
	LockGuard guard(_mutex);
	MemoryMonitorString internalName(
	    taskName.c_str(),
	    MemoryMonitorAllocator<char>(_usePSRAMBuffers)
	);
...
RegionStats ESPMemoryMonitor::captureRegion(uint32_t caps, MemoryRegion region) const {
	...
	if (_config.enableFragmentation && stats.freeBytes > 0 &&
	    stats.largestFreeBlock <= stats.freeBytes) {
		stats.fragmentation =
		    1.0f - static_cast<float>(stats.largestFreeBlock) / static_cast<float>(stats.freeBytes);
	} else {
...
MemoryMonitorVector<ESPMemoryMonitor::InternalTaskStackUsage>
ESPMemoryMonitor::captureStacks() const {
	...
	MemoryMonitorVector<TaskStatus_t> statuses{
	    MemoryMonitorAllocator<TaskStatus_t>(_usePSRAMBuffers)
	};
	statuses.resize(taskCount);
	uint32_t totalRuntime = 0;
	const UBaseType_t written =
	    uxTaskGetSystemState(statuses.data(), statuses.size(), &totalRuntime);
	statuses.resize(written);

	MemoryMonitorVector<InternalTaskStackUsage> usages{
	    MemoryMonitorAllocator<InternalTaskStackUsage>(_usePSRAMBuffers)
	};
	...
	for (const auto &status : statuses) {
		...
		usage.freeHighWaterBytes =
		    static_cast<size_t>(status.usStackHighWaterMark) * sizeof(StackType_t);
...
ScopeStats ESPMemoryMonitor::finalizeScope(const MemoryScope &scope) {
	...
	stats.deltaInternalBytes =
	    static_cast<int64_t>(scope._startInternal) - static_cast<int64_t>(internal.freeBytes);
	stats.deltaPsramBytes =
	    static_cast<int64_t>(scope._startPsram) - static_cast<int64_t>(psram.freeBytes);
	...
	MemoryMonitorVector<TagThresholdEvent> tagEvents{
	    MemoryMonitorAllocator<TagThresholdEvent>(_usePSRAMBuffers)
	};
	{
		LockGuard guard(_mutex);
...
LeakCheckResult
ESPMemoryMonitor::toPublicLeakCheckResult(const InternalLeakCheckResult &result) const {
	LeakCheckResult out{};
...
MemoryMonitorVector<ThresholdEvent>
ESPMemoryMonitor::evaluateThresholdsLocked(const InternalMemorySnapshot &snapshot) {
	...
	for (const auto &regionStats : snapshot.regions) {
		...
		const RegionThreshold &limits =
		    regionStats.region == MemoryRegion::Psram ? _config.psram : _config.internal;
...
ThresholdState ESPMemoryMonitor::evaluateState(
    ThresholdState current, const RegionThreshold &threshold, size_t freeBytes
) const {
	...
	case ThresholdState::Critical:
		if (threshold.criticalBytes == 0) {
			return threshold.warnBytes > 0 && freeBytes <= threshold.warnBytes
			           ? ThresholdState::Warn
...
void ESPMemoryMonitor::handleAllocEvent(
    size_t requestedBytes, uint32_t caps, const char *functionName
) {
...
void ESPMemoryMonitor::enrichSnapshotLocked(InternalMemorySnapshot &snapshot) const {
	...
	for (auto &region : snapshot.regions) {
		...
		MemoryMonitorVector<const RegionStats *> window{
		    MemoryMonitorAllocator<const RegionStats *>(_usePSRAMBuffers)
		};
		MemoryMonitorVector<uint64_t> timestamps{
		    MemoryMonitorAllocator<uint64_t>(_usePSRAMBuffers)
		};
		...
		if (targetWindow > 1) {
			size_t collected = 0;
			for (auto it = _history.rbegin(); it != _history.rend() && collected < targetWindow - 1;
			     ++it, ++collected) {
		...
		if (window.size() >= 2) {
			const float deltaFree = static_cast<float>(window.back()->freeBytes) -
			                        static_cast<float>(window.front()->freeBytes);
			const float deltaSeconds =
			    static_cast<float>(timestamps.back() - timestamps.front()) / 1'000'000.0f;
			...
			if (region.freeBytesSlope < 0.0f) {
				const RegionThreshold &limits =
				    region.region == MemoryRegion::Psram ? _config.psram : _config.internal;
				if (limits.warnBytes > 0 && region.freeBytes > limits.warnBytes) {
					const float seconds = (static_cast<float>(region.freeBytes) -
					                       static_cast<float>(limits.warnBytes)) /
				...
				if (limits.criticalBytes > 0 && region.freeBytes > limits.criticalBytes) {
					const float seconds = (static_cast<float>(region.freeBytes) -
					                       static_cast<float>(limits.criticalBytes)) /
					                      -region.freeBytesSlope;
...
void ESPMemoryMonitor::appendScopeLocked(const InternalScopeStats &stats) {
	...
	auto adjustUsage = [&](const InternalScopeStats &s, int direction) {
		if (s.tag == kInvalidMemoryTag || s.tag == 0 ||
		    static_cast<size_t>(s.tag) > _tagUsage.size()) {
			return;
		}
...
MemoryMonitorVector<TagThresholdEvent>
ESPMemoryMonitor::evaluateTagThresholdsLocked(const InternalScopeStats &stats) {
	MemoryMonitorVector<TagThresholdEvent> events{
	    MemoryMonitorAllocator<TagThresholdEvent>(_usePSRAMBuffers)
	};
...
ThresholdState ESPMemoryMonitor::evaluateRisingState(
    ThresholdState current, const TagBudget &budget, size_t usageBytes
) const {
	const size_t warnRecover =
	    saturatingSubtract(budget.warnBytes, _config.thresholdHysteresisBytes);
	const size_t criticalRecover =
	    saturatingSubtract(budget.criticalBytes, _config.thresholdHysteresisBytes);
...
StackState ESPMemoryMonitor::computeStackState(const InternalTaskStackUsage &usage) const {
	...
	if (it != _taskThresholds.end()) {
		...
		threshold.warnBytes =
		    static_cast<size_t>(_config.defaultTaskStackBytes * _config.stackWarnFraction);
		threshold.criticalBytes =
		    static_cast<size_t>(_config.defaultTaskStackBytes * _config.stackCriticalFraction);
	}
...
void ESPMemoryMonitor::trackTasksLocked(
    const InternalMemorySnapshot &snapshot, MemoryMonitorVector<TaskStackEvent> &events
) {
	...
	MemoryMonitorUnorderedMap<TaskHandle_t, InternalTaskStackUsage> current(
	    ...
	    std::hash<TaskHandle_t>{},
	    std::equal_to<TaskHandle_t>{},
	    MemoryMonitorAllocator<std::pair<const TaskHandle_t, InternalTaskStackUsage>>(
	        _usePSRAMBuffers
	    )
	);
	...
	for (const auto &[handle, prior] : _knownTasks) {
		if (current.find(handle) == current.end()) {
			events.push_back(
			    {toPublicTaskStackUsage(prior), computeStackState(prior), false, true}
			);
		}
	}
...
void ESPMemoryMonitor::resetOwnedContainers() {
	_history = MemoryMonitorDeque<InternalMemorySnapshot>(
	    MemoryMonitorAllocator<InternalMemorySnapshot>(_usePSRAMBuffers)
	);
	_scopeHistory = MemoryMonitorDeque<InternalScopeStats>(
	    MemoryMonitorAllocator<InternalScopeStats>(_usePSRAMBuffers)
	);
	_tagUsage = MemoryMonitorVector<InternalTagUsage>(
	    MemoryMonitorAllocator<InternalTagUsage>(_usePSRAMBuffers)
	);
	_tagBudgets =
	    MemoryMonitorVector<TagBudget>(MemoryMonitorAllocator<TagBudget>(_usePSRAMBuffers));
	_taskThresholds = MemoryMonitorUnorderedMap<MemoryMonitorString, TaskStackThreshold>(
	    0,
	    std::hash<MemoryMonitorString>{},
	    std::equal_to<MemoryMonitorString>{},
	    MemoryMonitorAllocator<std::pair<const MemoryMonitorString, TaskStackThreshold>>(
	        _usePSRAMBuffers
	    )
	);
	...
	_leakHistory = MemoryMonitorDeque<InternalLeakCheckResult>(
	    MemoryMonitorAllocator<InternalLeakCheckResult>(_usePSRAMBuffers)
	);
	_leakCheckpoints =
	    MemoryMonitorDeque<uint64_t>(MemoryMonitorAllocator<uint64_t>(_usePSRAMBuffers));
}

ESPMemoryMonitor::InternalLeakCheckResult
ESPMemoryMonitor::buildLeakCheckLocked(const std::string &label) {
	...
	const uint64_t startTs =
	    _leakCheckpoints.empty() ? _history.front().timestampUs : _leakCheckpoints.back();
	...
	auto computeAverages = [&](uint64_t from, uint64_t to) {
		...
		std::array<RegionAvg, 2> avgs{
		    {{MemoryRegion::Internal, 0.0f, 0.0f, 0}, {MemoryRegion::Psram, 0.0f, 0.0f, 0}}
		};
...
void ESPMemoryMonitor::runPanicHook() {
	...
	for (const auto &region : snapshot.regions) {
		...
		ESP_EARLY_LOGE(
		    kLogTag,
		    "panic snapshot %s free=%uB min=%uB largest=%uB frag=%.03f",
		    name,
