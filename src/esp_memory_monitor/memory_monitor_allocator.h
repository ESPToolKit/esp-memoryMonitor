...
class MemoryMonitorAllocator {
   public:
    ...
    template <typename U>
    bool operator==(const MemoryMonitorAllocator<U> &other) const noexcept {
   ...
   private:
    template <typename>
    friend class MemoryMonitorAllocator;

    bool _usePSRAMBuffers = false;
};

template <typename T>
using MemoryMonitorVector = std::vector<T, MemoryMonitorAllocator<T>>;

template <typename T>
using MemoryMonitorDeque = std::deque<T, MemoryMonitorAllocator<T>>;
