...
class MemoryScope {
  public:
	...
	ScopeStats end();
	bool active() const {
		return _monitor != nullptr && !_ended;
	}
...
class ESPMemoryMonitor {
  ...
  private:
	...
	struct InternalTaskStackUsage {
		...
		explicit InternalTaskStackUsage(bool usePSRAMBuffers)
		    : name(MemoryMonitorAllocator<char>(usePSRAMBuffers)) {
		}
	...
	struct InternalScopeStats {
		...
		explicit InternalScopeStats(bool usePSRAMBuffers)
		    : name(MemoryMonitorAllocator<char>(usePSRAMBuffers)) {
		}
	...
	struct InternalTagUsage {
		...
		explicit InternalTagUsage(bool usePSRAMBuffers)
		    : name(MemoryMonitorAllocator<char>(usePSRAMBuffers)) {
		}
	...
	evaluateThresholdsLocked(const InternalMemorySnapshot &snapshot);
	ThresholdState
	evaluateState(ThresholdState current, const RegionThreshold &threshold, size_t freeBytes) const;
	...
	evaluateTagThresholdsLocked(const InternalScopeStats &stats);
	ThresholdState
	evaluateRisingState(ThresholdState current, const TagBudget &budget, size_t usageBytes) const;
	StackState computeStackState(const InternalTaskStackUsage &usage) const;
	void trackTasksLocked(
